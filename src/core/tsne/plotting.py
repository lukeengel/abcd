...
def plot_qc_comparison(
    ...
    vmin = min(
        metadata["preqc"]["surface_holes"].min(),
        metadata["postqc"]["surface_holes"].min(),
    )
    ...
    scatter1 = axes[0].scatter(
        embeddings["preqc"][:, 0],
        embeddings["preqc"][:, 1],
        c=metadata["preqc"]["surface_holes"],
        cmap="gray_r",
        alpha=0.7,
        s=8,
        vmin=vmin,
    ...
    axes[0].set_title(
        f'Before QC (n = {len(embeddings["preqc"]):,})', fontweight="bold"
    )
    ...
    axes[1].scatter(
        embeddings["postqc"][:, 0],
        embeddings["postqc"][:, 1],
        c=metadata["postqc"]["surface_holes"],
        cmap="gray_r",
        alpha=0.7,
        s=8,
        vmin=vmin,
    ...
    axes[1].set_title(
        f'After QC (n = {len(embeddings["postqc"]):,})', fontweight="bold"
    )
    ...
    plt.suptitle(
        "Quality Control Impact on Data Distribution",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
    plt.savefig(
        plots_dir / f"qc_comparison_complexity{complexity}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
    ...
    pct = removed / preqc_count * 100
    print(
        f"QC removed {removed:,} samples ({pct:.1f}%) "
        f"with >{qc_threshold} surface defects"
    )
...
def plot_comparison(
    ...
    for i, (dtype, ax) in enumerate(zip([data_type1, data_type2], axes)):
        ...
        # For research groups, identify the control/baseline group (most common)
        if grouping == "research_groups":
            unique_groups, counts = np.unique(
                groups[~pd.isna(groups)], return_counts=True
            )
            baseline_group = unique_groups[
                np.argmax(counts)
            ]  # Most common group is baseline
        else:
            baseline_group = None
        ...
        for group in np.unique(groups):
            ...
            if grouping == "research_groups":
                # Baseline group gets subtle styling, others get highlighted
    ...
    plt.tight_layout()
    plt.savefig(
        plots_dir / f"{filename}_complexity{complexity}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
